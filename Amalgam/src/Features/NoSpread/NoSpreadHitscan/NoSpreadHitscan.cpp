...
void CNoSpreadHitscan::Draw(CTFPlayer* pLocal)
{
	...
	H::Draw.StringOutlined(fFont, x, y, cColor, Color_t(0, 0, 0, 255), align, std::format("Uptime {}", GetFormat(m_flServerTime)).c_str());
	H::Draw.StringOutlined(fFont, x, y += nTall, cColor, Color_t(0, 0, 0, 255), align, std::format("Mantissa step {}", m_flMantissaStep).c_str());
	if (Vars::Debug::Info.Value)
		H::Draw.StringOutlined(fFont, x, y += nTall, cColor, Color_t(0, 0, 0, 255), align, std::format("Delta {:.6f}", m_dTimeDelta).c_str());
}