...
void CBacktrack::Draw(CTFPlayer* pLocal)
{
	...
	if (flFake || Vars::Backtrack::Interp.Value > G::Lerp * 1000)
		H::Draw.StringOutlined(fFont, x, y, Vars::Menu::Theme::Active.Value, Color_t(0, 0, 0, 255), align, std::format("Ping {:.0f} (+ {:.0f}) ms", flLatency, flFake).c_str());
	else
		H::Draw.StringOutlined(fFont, x, y, Vars::Menu::Theme::Active.Value, Color_t(0, 0, 0, 255), align, std::format("Ping {:.0f} ms", flLatency).c_str());
	H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Color_t(0, 0, 0, 255), align, std::format("Scoreboard {} ms", iLatencyScoreboard).c_str());
}