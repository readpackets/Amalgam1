...
static inline void DrawVisuals(int iResult, Target_t& tTarget, std::vector<Vec3>& vPlayerPath, std::vector<Vec3>& vProjectilePath, std::vector<DrawBox_t>& vBoxes)
{
	if (G::Attacking == 1 || !Vars::Aimbot::General::AutoShoot.Value || iResult != 1)
	{
		...
		if (bPlayerPath || bProjectilePath || bBoxes || bRealPath)
		{
				...
				if (Vars::Visuals::Simulation::PlayerPath.Value == Vars::Visuals::Simulation::StyleEnum::Nitro) // is this a really bad way to do it? dont know
				{
					if (!vPlayerPath.empty())
					{
						...
						for (size_t i = 0; i < vPlayerPath.size(); i++)
						{
							...
							const float flGroundDistance = std::min(
								std::min(
									std::min(flMainDistance, flLeftDistance),
									std::min(flRightDistance, flFrontDistance)
								),
								flBackDistance
							);
						...
						for (auto& vSegment : vAirSegments)
						{
							...
							if (Vars::Colors::PlayerPathIgnoreZ.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::PlayerPathIgnoreZ.Value, Vars::Visuals::Simulation::StyleEnum::Separators);
							if (Vars::Colors::PlayerPath.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::PlayerPath.Value, Vars::Visuals::Simulation::StyleEnum::Separators, true);
						}
			...
			if (bProjectilePath)
			{
				if (Vars::Visuals::Simulation::ProjectilePath.Value == Vars::Visuals::Simulation::StyleEnum::Nitro) // Nitro style for projectile path
				{
					if (!vProjectilePath.empty())
					{
						...
						for (size_t i = 0; i < vProjectilePath.size(); i++)
						{
							const float flProjectileHeight = 10.0f; // Approximate height for projectiles
							Vec3 vTraceStart = vProjectilePath[i] + Vec3(0, 0, flProjectileHeight * 0.33f);
							Vec3 vTraceEnd = vProjectilePath[i] - Vec3(0, 0, 512.0f);
							...
							const float flOffset = 8.0f; // Smaller offset for projectiles
							
							Vec3 vForward, vRight;
							if (i > 0)
							{
								Vec3 vDir = (vProjectilePath[i] - vProjectilePath[i-1]).Normalized2D();
								vForward = Vec3(vDir.x, vDir.y, 0.0f);
								vRight = Vec3(-vDir.y, vDir.x, 0.0f);
							}
							else
							...
							const float flMainDistance = (vProjectilePath[i] - trace.endpos).Length();
							const float flLeftDistance = (vProjectilePath[i] - traceLeft.endpos).Length();
							const float flRightDistance = (vProjectilePath[i] - traceRight.endpos).Length();
							const float flFrontDistance = (vProjectilePath[i] - traceFront.endpos).Length();
							const float flBackDistance = (vProjectilePath[i] - traceBack.endpos).Length();
							...
							bool bVelocityBasedAir = false;
							if (i > 0 && i < vProjectilePath.size() - 1)
							{
								const Vec3 vPrevVelocity = (vProjectilePath[i] - vProjectilePath[i-1]) * (1.0f / TICK_INTERVAL);
								const Vec3 vNextVelocity = (vProjectilePath[i+1] - vProjectilePath[i]) * (1.0f / TICK_INTERVAL);
								const Vec3 vAcceleration = (vNextVelocity - vPrevVelocity) * (1.0f / TICK_INTERVAL);
								...
								bVelocityBasedAir = 
									(fabs(vAcceleration.z) > 350.0f) || 
									(fabs(vNextVelocity.z) > 120.0f && flGroundDistance > 24.0f) ||
									(flDirectionChange > 0.3f && vAcceleration2D.Length() > 500.0f && flGroundDistance > 12.0f) ||
									(vPrevVelocity.z > 0.0f && vNextVelocity.z > 0.0f && flGroundDistance > 18.0f);
							}
							
							const float flGroundThreshold = 12.0f; // Smaller threshold for projectiles
							const bool bOnGround = (bTraceValid && flGroundDistance <= flGroundThreshold && !bVelocityBasedAir);
							const bool bInAir = !bOnGround;
							...
							{
								const Vec3& vPrevPoint = vCurrentSegment.back();
								const Vec3& vNextPoint = vProjectilePath[i];
							...
							vCurrentSegment.push_back(vProjectilePath[i]);
							bLastInAir = bInAir;
						}
						...
						{
							const float flDuration = Vars::Visuals::Simulation::Timed.Value ? 
								-int(vSegment.size()) - TIME_TO_TICKS(F::Backtrack.GetReal()) : 
								I::GlobalVars->curtime + Vars::Visuals::Simulation::DrawDuration.Value;
							
							if (Vars::Colors::ProjectilePathIgnoreZ.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::ProjectilePathIgnoreZ.Value, Vars::Visuals::Simulation::StyleEnum::Separators);
							if (Vars::Colors::ProjectilePath.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::ProjectilePath.Value, Vars::Visuals::Simulation::StyleEnum::Separators, true);
						}
						...
						for (auto& vSegment : vGroundSegments)
						{
							...
							if (Vars::Colors::ProjectilePathIgnoreZ.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::ProjectilePathIgnoreZ.Value, Vars::Visuals::Simulation::StyleEnum::Line);
							if (Vars::Colors::ProjectilePath.Value.a)
								G::PathStorage.emplace_back(vSegment, flDuration, Vars::Colors::ProjectilePath.Value, Vars::Visuals::Simulation::StyleEnum::Line, true);
						}
					}
				}
				else
